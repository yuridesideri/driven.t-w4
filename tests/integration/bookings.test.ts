  ...
  describe("when token is valid", () => {
    ...
    describe("when user has no proper business rules respond with FORBIDDEN(403)", () => {
      it("when user doesnt have an enrollment yet", async () => {
        ...
        const booking = await createBooking(user.id, room.id);
        const response = await server.put(`/bookings/${booking.id}`).set("Authorization", `Bearer ${token}`).send({
          roomId: room2.id,
        });
      ...
      it("when user is not the owner of the booking", async () => {
        ...
        const createdBooking2 = await createBooking(user1.id, room2.id);

        const response = await server
          .put(`/bookings/${createdBooking2.id}`)
          .set("Authorization", `Bearer ${token}`)
          .send({
            roomId: room2.id,
          });
    ...
    it("should respond with with NOT_FOUND(404) when bookingId does not exist", async () => {
      ...
      const response = await server.put("/bookings/0").set("Authorization", `Bearer ${token}`).send({
        roomId: room.id,
      });
    ...
    it("should respond with OK(200) when roomId and ticket are valid", async () => {
      ...
      const response = await server.put(`/bookings/${createdBooking.id}`).set("Authorization", `Bearer ${token}`).send({
        roomId: room2.id,
      });
